    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-xl-12">
                ...
                <div class="mb-3">
                    <a href="{{route('create.university')}}"
                        class="btn btn-primary btn-sm">Add University</a>
                </div>
                                            ...
                                            @foreach ($university as $item)
                                                <tr>
                                                    ...
                                                    @php
                                                        ...
                                                        $state = App\Models\State::find($item->state);
                                                    @endphp
                                                    <td>{{ $country->name }}</td>
                                                    <td>{{ $state->name }}</td>
                                                    <td>{{ ucfirst($item->university_name ) }}</td>
                                                    <td>{{ ucfirst($item->university_slug) }}</td>
                                                    <td>{{ ucfirst($item->university_location) }}</td>
                                                    <td>{{ ucfirst($item->city) }}</td>
                                                    <td>{{ ucfirst($item->zip) }}</td>
                                                    <td>{{ ucfirst($item->phone_number) }}</td>
                                                    <td>{{ ucfirst($item->email) }}</td>
                                                    <td>{{ ucfirst($item->website) }}</td>
                                                    <td class="text-nowrap">
                                                        <a title="Edit"
                                                            href="{{route('edit.university',[$item->id])}}"
                                                    ...
                                                    <td class="text-nowrap">
                                                        <a title="Delete"
                                                            href="{{route('delete.university',[$item->id])}}"
                                                            class="btn btn-warning btn-sm content-icon">
                                ...
                                <div class="d-flex align-items-center justify-content-between flex-wrap">
                                    ...
                                    <nav aria-label="Page navigation example mb-2">
                                        <ul class="pagination mb-2 mb-sm-0">
                                            <li class="page-item">
                                                <a class="page-link" href="{{ $university->previousPageUrl() }}">
                                                    <i class="fa-solid fa-angle-left"></i>
                                                </a>
                                            </li>

                                            <!-- Display each page link -->
                                            @for ($i = 1; $i <= $university->lastPage(); $i++)
                                                <li class="page-item {{ $university->currentPage() == $i ? 'active' : '' }}">
                                                    <a class="page-link"
                                                        href="{{ $university->url($i) }}">{{ $i }}</a>
                                                </li>
                                            @endfor

                                            <li class="page-item">
                                                <a class="page-link" href="{{ $university->nextPageUrl() }}">
